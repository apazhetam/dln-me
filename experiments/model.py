...
def get_model(
    ...
    assert is_valid_config(
        config, dataset
    ), f"Invalid config {config} for dataset {dataset}"
...
class DLN(nn.Module):
    def __init__(self, config: DLNConfig):
        ...
        if self.prog_freeze:
            ...
            self.freeze_schedule = self.get_prog_schedule(
                layer_indices,
                config.freeze_start_frac,
                1,
                config.num_epochs,
                config.freeze_reverse,
            )
        ...
        if self.prog_prune:
            ...
            self.prune_schedule = self.get_prog_schedule(
                layer_indices,
                config.prune_start_frac,
                config.prune_end_frac,
                config.num_epochs,
                config.prune_reverse,
            )
        ...
        for i in range(self.depth):
            ...
            if isinstance(layer, LogicLayer):
                if (
                    self.concat_input
                    and i > 0
                    and isinstance(self.layers[i - 1], LogicLayer)
                ):
    ...
    def freeze_params(self):
        if not self.prog_freeze or (
            self.epochs_trained not in self.freeze_schedule.keys()
        ):
            return
        freeze_list = self.freeze_schedule[self.epochs_trained]
        logging.debug(
            f"\tafter {self.epochs_trained} epochs, freeze_list: {freeze_list}"
        )
    ...
    @torch.no_grad()
    def prune(self):
        if not self.prog_prune or (
            self.epochs_trained not in self.prune_schedule.keys()
        ):
            return
        ...
        prune_list = [
            (idx, self.idx2next_layer_idx[idx] or float("inf")) for idx in prune_list
        ]
        prune_list.sort(key=itemgetter(1))
        prune_list = [
            [x[0] for x in g] for _, g in groupby(prune_list, key=itemgetter(1))
        ]
        ...
        for cur_prune_list in prune_list:
            ...
            next_layer = (
                self.layers[next_layer_idx] if next_layer_idx is not None else None
            )
            ...
            for idx in cur_prune_list:
                ...
                output_mask = layer.prune_neurons(
                    min_neuron_left=self.prune_min_neuron_left,
            ...
            if next_layer is not None:
                if self.concat_bool and isinstance(next_layer, LogicLayer):
                    mask_list.append(
                        torch.ones_like(self.bool_constants, dtype=torch.bool).squeeze()
                    )
                logging.debug(
                    f"\t\tupdating input mask for layer {next_layer_idx}, {next_layer}"
                )
                next_layer.add_input_mask(torch.cat(mask_list, dim=0))

    def get_prog_schedule(
        self, layer_indices, start_frac, end_frac, num_epochs, reverse
    ):
        ...
        action_epochs = (
            np.linspace(start_frac, end_frac, len(idx_to_schedule.keys())) * num_epochs
        ).astype(int)
    ...
    def get_num_neurons(self):
        return sum(
            ...
            if not isinstance(l, ThresholdLayer)
        )
    ...
    def get_ops_dict(self):
        ...
        ops_dict = {
            k: dict(v) for k, v in ops_dict.items()
    ...
    def load_model(self, filepath, device=None):
        ...
        for i, layer in enumerate(self.layers):
            for name, value in state_dict.items():
                if (
                    name not in ["device"]
                    and name.startswith(f"layers.{i}.")
                    and hasattr(layer, name.split(".")[-1])
                ) and not isinstance(value, torch.Tensor):
    ...
    def print_layers(self):
        ...
        for i in range(len(self.layers)):
            logging.debug(
                f"idx: {i}; layer: {self.layers[i]}, depth: {self.idx2depth[i]}, next_layer_idx: {self.idx2next_layer_idx[i]}"
            )
        logging.debug("-" * 10)
...
def get_hidden_dims(
    ...
    hidden_dims = np.geomspace(
        config["first_hl_size"],
...
def is_valid_config(
    ...
    if config.get("num_phase_rounds", None) is not None:
        if (
            config["num_phase_rounds"] <= 0
            or config["num_epochs"] < config["num_phase_rounds"] * 2
        ):
            return False
...
def evaluate_computation(
    ...
    try:
        cmd_return = subprocess.run(
            cmd, capture_output=True, text=True, timeout=simplify_timeout_second
        )
        if cmd_return.returncode == 0 and cmd_return.stdout:
            cmd_return = json.loads(cmd_return.stdout)
        else:
            logging.error(
                f"Error in executing the visualization script for {sympy_code_path}."
            )
            cmd_return = None
    except subprocess.TimeoutExpired:
        logging.error(
            f"Timeout in executing the visualization script for {sympy_code_path}."
        )
        cmd_return = None
    ...
    return {
        "num_hl_ops": num_hl_ops,
        "num_bg_ops": num_bg_ops,
        "num_params": num_params,
        "num_disk_bits": num_disk_bits,
